...
// CLASSE PRINCIPAL
class ProcessaResgataFiliais extends REST
{
    protected $msg=array();
    ...
    function executar($param = null)
    {
        ...
        try {
            $filewsdl = 'marcelo.wolff--Filiais1.0.0.wsdl';
            ...
            $dtenvio=time();
			$result = $soapClient->consultarFilial($arguments);

			if (isset($result->Filial)) {
				$error = NULL;
				$str_array = (array) $result->Filial;
				$arr_res = array();
				$arr_res['error']  = $error;
				...
				$arr_res=array();
				$arr_res['error']  = array("msg" => "Branch not found");
	...
	$obj = new ProcessaResgataFiliais();
